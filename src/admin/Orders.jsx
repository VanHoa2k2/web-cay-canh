  ...
  useEffect(() => {
    ...
    const printBill = async (id) => {
      ...
      if (!result) {
        ...
        toast.success("In hóa đơn sản phẩm thành công");
            ...
            {bills.map((bill, i) => (
              <Row key={i} className="bill__container">
                ...
                <Col lg="9">
                  ...
                  <h5>Tổng Cộng: {(bill.totalAmount + 30000).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} VNĐ</h5>
                </Col>

                <Col lg="3">
                  <button onClick={() => printBill(bill._id)} className="btn btn-success">Xuất Hóa Đơn</button>
                </Col>
              </Row>
            ))}
